package org.springinincheon.chat.config;
...
public class WebSockConfig implements WebSocketConfigurer {
    ...
    public void registerWebSocketHandlers(WebSocketHandlerRegistry registry){   // WebSocketHandlerRegistry: Handler의 요청(URL 경로 등)에 대한 설정을 구성할 수 있는 메소드를 제공하는 I/F
        registry.addHandler(webSocketHandler, "/chat").setAllowedOrigins("*");      // setAllowedOrigins : Origin 내 총 3가지 요소(protocol, host, port)가 동일한 허용된 Origin 요청만 수락. "*" 사용시 모든 Origin 허용      //  .withSockJS() 추가 시, SockJS 사용 가능
    }
}
